...
// Renderer
export class AnatomyRenderer extends PoseRenderer {
  // Scene
  ...
  constructor(
    ...
    mode?: "fit" | "crop",
    mirror?: boolean,
    protected url = "skeleton.glb",
    protected layer?: OutfitParams
  ) {
    super(container, mode, mirror);
    this.plugin = new PoseOutfitPlugin(undefined, layer);
  ...
  protected async setupScene(scene: three.Scene) {
    ...
    const geometry = new TextGeometry("ANATOMY AR", {
      font: font,
      size: 5,
      height: 2,
      bevelSize: 0.3,
  ...
  async setModel(url: string) {
    return this.setLayer(url, this.layer);
  }

  // Set layer to render
  async setLayer(url: string, layer?: OutfitParams) {
    ...
    this.plugin.setOutfit(this.model, layer);
  }
